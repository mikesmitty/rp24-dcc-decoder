...
func TestSetCVCommand(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			buf:    []byte{0b0100, 9, 0x42}, // op=01, cv=10, data=0x42
		...
		},
		{
			...
			buf:    []byte{0b1100, 9, 0x42}, // op=11, cv=10, data=0x42
		...
		},
		{
			...
			buf:    []byte{0b1000, 9, 0b11101001}, // op=10, cv=10, verify bit 1 at position 1
		...
		},
		{
			...
			buf:    []byte{0b1000, 9, 0b11111001}, // op=10, cv=10, write bit 1 at position 1
			mockCV: cv.NewMockHandler(true, testCVConfirm([]uint16{10}, []uint8{0})),
			expect: true,
		},
		{
		...
		},
		{
			...
			buf:    []byte{0b1000, 9, 0x01}, // op=10, cv=10, invalid bit command (missing 111 prefix)
			mockCV: cv.NewMockHandler(true, testCVConfirm([]uint16{10}, []uint8{0})),
			expect: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := m.cvCommand(0, tt.buf)
			if got != tt.expect {
				t.Errorf("cvCommand() = %v, want %v", got, tt.expect)
			}
		})
	}
}
